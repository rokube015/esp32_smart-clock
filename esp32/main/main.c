...
const char* SCD40_TAG = "scd40_tag";
...
esp_err_t get_scd_40_serial_number(uint64_t* pserial_number, i2c_master_dev_handle_t* pdev_handle){
  esp_err_t r = ESP_OK;
  union{
    uint8_t arr[9];
    struct{
  ...
  r = i2c_master_transmit(*pdev_handle, serial_number_get_command, sizeof(serial_number_get_command), -1);
  ESP_ERROR_CHECK(r);
  if(r != ESP_OK){
  ...
  r = i2c_master_receive(*pdev_handle, serial_number.arr, sizeof(serial_number.arr), -1);
  ESP_ERROR_CHECK(r);
  if(r != ESP_OK) {
...
void app_main(void){
  ...
  get_scd_40_serial_number(&serial_number, &dev_handle);

  ESP_LOGI(SCD40_TAG, "SCD40 Serial Number:%llu", serial_number);
  while(1);
