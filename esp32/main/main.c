...
void app_main(void){
  ...
  ESP_LOGI(MAIN_TAG, "The current date/time in Japan is: %s %s", day_buf, time_buf);
  ...
  while(1){
    ...
    if(r == ESP_OK){
      ...
      if(r == ESP_OK){
        ...
        strftime(day_buf, sizeof(day_buf), "%Y/%m/%d", &timeinfo);
        strftime(time_buf, sizeof(time_buf), "%H:%M:%S", &timeinfo);
        ESP_LOGI(MAIN_TAG, "%s %s co2:%d, temperature:%f, humidity:%f",
                  day_buf, time_buf, scd40_value.co2, scd40_value.temperature, scd40_value.relative_humidity);
      }
    }
    if(r == ESP_OK){
      vTaskDelay(5000/ portTICK_PERIOD_MS);
